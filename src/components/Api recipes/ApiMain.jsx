...
export default function ApiMain() {
  const { apiRecpieRan, sutapiRecpieRan, apiData, setapiData } =
  ...
  function Search() {
    if (product) {
      fetch(
        `https://api.spoonacular.com/recipes/complexSearch?query=${product}&apiKey=${
          ...
          {apiData != null
            ? apiData.map((item, i) => (
                <div className=" mb-3" key={i}>
                  {item.image ? <ApiCard key={i} item={item} /> : ""}
                </div>
              ))
